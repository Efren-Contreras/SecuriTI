...
<body class="text-zoom" style="background-image: linear-gradient(to bottom, #5faaf2, #6db3e7, #82badb, #9bc0d1, #b5c4c9);">
    <%
        if (deviceType.equals("WorkStation")){
        %>
            <%@include file="devices/editarWS.jsp" %>
        <%
        }
        else if (deviceType.equals("Server")){
        %>
            <%@include file="devices/editarSV.jsp" %>
        <%
        }
        else {
        %>
